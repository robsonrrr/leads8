...
Multiple Where and Having clauses are added to the query with Boolean operators connecting each expression. The default operator for both methods is AND which is the same as the and* prefixed method. The OR operator can be specified by prefixing the methods with or*. Where and Having clauses can be nested or grouped by post fixing either method with \_open and then followed by a method with a \_close.
