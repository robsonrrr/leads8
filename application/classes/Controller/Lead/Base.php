...
class Controller_Lead_Base extends Controller_Website  {
    ...
    public function get_lead_base( $leadID )
    {
        ...
        $sql= sprintf("SELECT sCart.*, nop.id_nop, nop.cfop, Emitentes.EmitentePOID, Emitentes.UF, Emitentes.Fantasia as local,
                              ...
                              clientes.icms, clientes.regime_especial, clientes.suframa, clientes.regime_fiscal, clientes.difal, clientes.id
                         FROM sCart 
        ...
        if ( isset( $lead[0]) )
        {
            ...
            $lead['Cliente']['clienteID']                = $lead['id'];
            ...
            $lead['Cliente']['clienteDifal']             = $lead['difal'];
        }
    ...
    public function get_solr_query( $id, $segmento = false)
    {
        ...
        $fl= "id,produto*,segmento*,categoria*,catalogo*,catalogoSimilarPDF,catalogoOriginalPDFmarcaSeo";
        ...
        $url = $_ENV['solr']."/catalogo/select?q=produtoPOID:".$id."&fl=".$fl."&indent=on&wt=json&rows=1".$fq;
    ...
    {   
        ...
        if ( 5 == $segmento )
        {
            ...
            {
                //Diretoria
                if  ( $discount > 0 and ( $_SESSION['MM_Userid'] == '188' or $_SESSION['MM_Userid'] == '34' or $_SESSION['MM_Nivel'] > 4 ) )
                {
                    return $discount;
                }
    ...
    public function get_tax( $lead, $product )
    {
        ...
        // $url = sprintf( $_SERVER["HTTP_X_FORWARDED_PROTO"].'://'.$_SERVER["HTTP_X_FORWARDED_HOST"].'/tax/index' );
      
        // $auth = base64_encode( $_ENV['auth'] );
        
        // $postdata = http_build_query(
        //     array(
        //         'emitenteUF' => $lead['Emitente']["emitenteUF"],
        //     )
        // );
        ...
        $arr["clienteDifal"]            = $lead['Cliente']["clienteDifal"];
        $arr["clientePOID"]             = $lead['Cliente']['clienteID'];
        ...
        $url = sprintf( $_ENV['tax'].'/index?%s',$vars );
        // $url="https://dev.office.internut.com.br/tax/index/?$vars";
        $ch = curl_init();
    ...
    public function get_price( $product, $customer, $unity = false, $terms= false , $lead = false, $segmentID = false)
    {
        ...
        $url = sprintf( $_ENV['price'].'/%s/%s', $product, $customer);
    ...
    public function get_price_group( $ids, $unity, $customer, $terms = false, $segmentID = false )
    {
        ...
        $url = sprintf( $_ENV['price'].'/group/%s/%s?ids=%s', $unity, $customer, $ids);
    ...
    {
        $url = sprintf( $_ENV['stock'].'/group/%s/%s?ids=%s', 0, $unity, $ids );
    ...
    public function get_stock( $id, $unity = false )
    {
        ...
        if ( $unity )
        {
            ...
            $url = sprintf( $_ENV['stock'].'/get/%s/%', $id, $unity );
        }else{
            //$url = 'http://vallery-stock/get/'.$id;
            $url = sprintf( $_ENV['stock'].'/get/%s/', $id );
        }
